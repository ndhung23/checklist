...
/**
 * AdminPage - Dashboard for admin users
 * Shows ALL users' checks with full CRUD, filters, and user selection
 */
const AdminPage = () => {
  const { user } = useAuth();
  ...
  /** Fetch all checks (admin endpoint) */
  const fetchChecks = useCallback(async () => {
    try {
      const res = await axiosClient.get("/admin/checks?role=admin");
      setChecks(res.data);
    } catch (err) {
      console.error("Error fetching admin checks:", err);
    } finally {
      setLoading(false);
    }
  }, []);

  /** Fetch users list for filters and add modal */
  ...
  useEffect(() => {
    ...
    fetchUsers();
    fetchCategories();
  }, [fetchChecks, fetchUsers, fetchCategories]);
  ...
  const filteredChecks = checks.filter((c) => {
    ...
    if (filterUser && c.userId !== parseInt(filterUser)) return false;
    if (searchText) {
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-header">
        ...
        <p>Quản lý toàn bộ checklist của tất cả người dùng</p>
      </div>
      ...
      {/* Add Modal - Admin can assign to any user */}
      <Modal
        show={showAddModal}
